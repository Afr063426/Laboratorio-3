...
La ganancias de capital son igual a $\mathbf{R_j}=\frac{S_j}{S_{j-1}}-1$ con $j=1,...,n$, que también son variables aleatorias i.i.d. y son independientes.
...
* Se supone el activo paga un dividendo constante, continuo y proporcional al precio final de todo el tiempo, desde $t_0$ hasta $t_f$: De tal forma que
...
Ahora se destaca que el retorno se puede obtener mediante la ganancia de capital y la proporcion de dividendo $qh_j$ esto es $R(t_0,t_1)=\mathbf{R}_1+qhn$ entonces

\[
\frac{\mathbb{E}(\mathbf{R_1})}{h_n}=\frac{\mathbb{E}(R(t_0,t_1)-q)}{h_n}\to m-q
\]

$m$ y $q$ se asumen dados y de tal forma que $m$ corresponde a la tasa libre de riesgo o de retorno esperado.
...
\]
Se asume que $\sigma^2_n\to \sigma_2>0$ cuando $n\to \infty$. Esto se le llama la volatilidad del precio de la seguridad. Ahora los inversores pueden ser más o menos adversos al riesgo. Para el caso de los inversores con riesgo neutral se cumplen las siguientes ecuaciones para $n$ lo suficientemente grande.
\[
...
\[
P=\frac{\sum_{i=0}^{a-1}\left(\frac{n!}{i!(n-i)!}p^{i}(1-p)^{n-i}u^id^{n-i}S_0-K\}\right)}{(1+r)^{\tau}}
\]

Ahora sin embargo hasta este punto se ha trabajado de forma discreta y no se ha dado el salto hacia el caso continuo al menos en esta sección, y es cierto que se ha hablado del árbol 
