...
precioC<-function(K,S_0,n,hn,volatilidad,r,q,t){
  parametros<-parametros(K,S_0,n,hn,volatilidad,r,q)
  sum(choose(n,c(parametros[1]:n))*parametros[2]^(c(parametros[1]:n))*(1-parametros[2])^(n-c(parametros[1]:n))*(parametros[3]^(parametros[1]:n)*(1/parametros[3])^(n-c(parametros[1]:n))*S_0-K))*exp(-t*r) ### Formula para precio de de las opciones de compra
}
...
Precios<-Precios[Precios$Fecha>=as.Date("01.01.2021",format="%d.%m.%Y"),]
...
hnTrimetral<-0.4/as.numeric(nTrimestral)
...
precioOpcionCTrimestral<-precioC(K,K,nTrimestral,hnTrimetral,sigma,svensson(parametrosModelo,1/4),1/4,0)
precioOpcionPTrimestral<-precioP(K,K,nTrimestral,hnTrimetral,sigma,svensson(parametrosModelo,1/4),1/4,0)
### Opciones put y call para el  caso semestral
precioOpcionCSemestral<-precioC(K,K,nTrimestral,hnTrimetral,sigma,svensson(parametrosModelo,1/2),1/2,0)
precioOpcionPSemestral<-precioP(K,K,nTrimestral,hnTrimetral,sigma,svensson(parametrosModelo,1/2),1/2,0)
### Opciones put y call para el caso anual
precioOpcionCAnual<-precioC(K,K,nTrimestral,hnTrimetral,sigma,svensson(parametrosModelo,1),1,0)
precioOpcionPAnual<-precioP(K,K,nTrimestral,hnTrimetral,sigma,svensson(parametrosModelo,1),1,0)
...
```{r}
arbolrTri<-t(arbolBinarioM(nTrimestral,hnTrimetral,sigma,K))
arbolrTri[is.na(arbolrTri)]=""
arbolrTri%>%kbl()%>%kable_material()%>%kable_styling(full_width = F)%>%scroll_box(width = "100%",height = "500px")
```

\textit{\textbf{El árbol con el precio para la opción de compra del activo subyacente es el siguiente}}
