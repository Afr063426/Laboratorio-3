...
Se tiene que las acciones se cotizan en la bolsa de valores de Costa Rica, por lo que se tiene que tener presenta los días hábiles con los que se cuentan para las transacciones, teniendo así la siguiente table con los días feriados:\\
```{r}
tablaKableConDiaNB
```
...
```
Primero los valores obtenidos para $a$, $p$ y $u$ son para el caso a tres meses `r valoresTrimestral`, a seis meses `r valoresSemestral` y a un año `r valoresAnual`. El árbol para el caso trimestral se tiene que el árbol es el siguiente
...
Los precios respectivos para las opciones se resument en la siguiente tabla
```{r}
