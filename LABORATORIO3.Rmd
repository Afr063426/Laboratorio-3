...
<caption><center><strong>Gráfico 2: Precio de las acciones de Holcim Costa Rica S.A.</strong></center></caption>
```{r}
...
<caption><center><strong>Gráfico 3: Precio de las acciones de Holcim Costa Rica S.A. 01/03/2020-17/06/2021</strong></center></caption>
```{r}
...
<caption><center><strong>Gráfico 4: Curva de Rendimientos (Anual), _Svensson_ </strong></center></caption>
```{r}
...
##Se procede a calcular los retornos logarítmicos de los precios de las accioneshttp://127.0.0.1:32361/rmd_output/4/#conclusi%C3%B3n. Para esto, primero se calculan los rendimientos brutos semanales, seguidamente del logaritmo de estos
...
<caption><center><strong>Árbol Binomial de Opción de Compra</strong></center></caption>
```{tikz, fig.ext = 'png'}
\usetikzlibrary{arrows}
\usetikzlibrary{trees}
\usetikzlibrary{matrix}
...
<caption><center><strong>"Árbol Binomial de Opción de Venta"</strong></center></caption>
```{tikz, fig.ext = 'png'}
\usetikzlibrary{arrows}
\usetikzlibrary{trees}
\usetikzlibrary{matrix}
...
Es importante destacar que se trabaja de forma anualizada de tal forma que si el tiempo para el vencimiento de una opción es de tres meses entonces $t_f=0.25$, seis meses $t_f=0.5$ y un año $t_f=1$. El riesgo neutral se calcula a través de una interpolación de las tasas (anualizadas) con las que se cuenta mediante el modelo de _Svensson_ a partir para el cual se obtiene los parámetros que pueden ser usados para calcular $y(t)$, y así encontrar el factor de descuento

\[
e^{-y(t_f)h_n}
\]

Además se tiene que las acciones de Holcim S.A. pagan dividendos en fechas establecidas. Sin embargo estos no se basan en una composición continua de pagos de los mismos y se establece en un rango de fechas, por lo que se conviene que salvo la opción tenga vencimiento en los rangos de fechas establecidos para el pago de los dividendos

\[
q=0
\]
Y en caso de que la opción se establezca en una fecha donde se pagan dividendos poseedor de la acción hasta fecha es el que tiene el derecho al dividendo. 
Basado en los registros con los que se cuenta de la empresa se supone los siguientes pagos de dividendos serán oficializados el tanto monto como fecha de pago se harán el 21 de diciembre de 2021 y el 22 de abril del 2022.

Además la volatilidad a sido calculada de forma semanal ya que eran los datos con mayore recurrencia con los que se contaba, con esto se calculó la volatilidad anual
\[
\sqrt{52}\cdot\sigma_{\text{Semanal}}=\sigma_{\text{Anual}}
\]
Donde $\sigma_{\text{Semanal}}$ es calculado mediante el cálculo de los rendimientos logarítmicos en cada semana y la función de desviación _sd_ de $\mathbf{R}$. La cantidad de pasos hasta el vencimiento de la opciónse establecieron de acuerdo a la cantidad de días entre el día que la opción fue escrita y la fecha de vencimiento de la misma, teniendo en cuenta los días no bursátiles según la Bolsa Nacional de Valores de Costa Rica.
