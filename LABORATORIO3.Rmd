...
tablaNotacion%>%kbl()%>%kable_material()%>%kable_styling(full_width = F)
```
...
tablaLongCall%>%kbl(col.names = c('Tiempo/Escenario','$S_T$< $K$',"$S_T \\geq K$"))%>%kable_material()
```
...
tablaLongCall%>%kbl(col.names = c("Tiempo/Escenario","$S_T \\leq K$","$S_T$ > $K$"))%>%kable_material()
```
...
El árbol con el precio para la opción de compra del activo subyacente es el siguiente
