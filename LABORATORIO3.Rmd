---
title: "Valoración de opciones para la compra y venta de acciones de Holcim Costa Rica S.A."
date: "`r Sys.Date()`"
autor: "Cervantes Artavia Joshua B81952 \\ Gonzalez Alfaro David B93327 \\ Sabater Guzmán Daniel"
output:
...
Cervantes Artavia Joshua B81952 

Gonzalez Alfaro David B93327

Sabater Guzmán Daniel B87096
    
    
# Introducción

Las opciones de compra y venta de activos financieros son instrumentos utilizados alrededor del mundo por inversionistas que buscan incrementar su riqueza o cubrirse del riesgo.  Así que en el presente trabajo se va expone el modelo de Cox, Ross y Rubinstein (1979), mismo que se aplicara sobre el activo denominado acciones de Holcim Costa Rica, para obtener el resultado de las opciones de compra y venta.

El trabajo abarcara un marco teórico detallado, que incluye aspectos relevantes para la aplicación del modelo como lo son la curva de rendimiento soberana, aspectos estadísticos, supuestos para trabajar con árboles binomiales, descripción de las opciones y presiones de las mismas.

También se incluye una sección metodológica y de análisis de los datos implementados. En esta última, se lleva a cabo una presentación de los datos y características importantes de los mismos. Por su parte, se dedica una sección al análisis del árbol y su implementación, para clarificar el proceso seguido para su obtención.

Todo lo anterior se sigue como fin para lograr establecer los resultados de la aplicación del modelo con los datos presentados, teniendo como resultados las opciones de compra y venta a tres meses, seis meses y un año.
...
La primera caída observable se debe una a la investigación judicial llevada a cabo por la fiscalía, donde se allanaron las oficinas de las empresas ,esto debido a un posible tráfico de influencias y enriquecimiento ilícito, como lo establece Solano, H. (2019).

Mientras que la caída a finales del año 2019 he inicios del 2020, es producto de la especulación en los mercados y la venta de activos, para la obtención de dinero líquido, causado por la pandemia del COVID-19 y su afectación en los mercados. 
...
Siendo así un total de `r length(Precios$Fecha)` observaciones no diarias, sin embargo, se cuenta con `r length(Precios_semanales)` observaciones semanales. Se procede a calcular la volatilidad semanal, teniendo como resultado `r volatilidad_semanal` y multiplicando por la raíz cuadrada de la cantidad de semanas que tiene un año (52), se obtienen una volatilidad anual de `r volatilidad_anual`.
...
A continuación se presenta una tabla con la matriz, que representa el árbol binomial para el caso de los precios a tres meses con `r nTrimestral` pasos.
...
Basándose en la tabla 7 (árbol), se presenta una tabla donde se encuentra la matriz de los posibles precios en cada uno de los pasos de una opción de compra a tres meses.
...
Se presentan los precios de las opciones a tres meses, seis meses y un año, en la siguiente tabla:
...
Con base en los resultados obtenidos y a partir de los análisis mostrados, se concluye que es posible calcular las opciones de compra y venta del activo subyacente utilizado con la teoría y los modelos implementados. Se logra examinar e interpretar los datos recopilados, así como aplicar los modelos de Cox, Ross y Rubinstein y el de Svensson para estimar los posibles precios futuros de las acciones y sus respectivas variables aleatorias.

Además, se infiere que las opciones de compra y venta de las acciones analizadas son relativamente bajas, esto debido a que en el año estudiado la volatilidad y los rendimientos fueron relativamente bajos. Lo anterior repercute directamente en la probabilidad de que el valor de la acción suba o baje, y esto mismo incide en el cálculo de las opciones. 

Por ende, se concluye también que existe una relación positiva entre la volatilidad y probabilidad de variación los activos financieros con los valores de las opciones de compra y venta de estos. Esta afirmación se puede evidenciar mediante la fórmula par calcular estas opciones.

Finalmente, se deduce que el riesgo asociado a este activo financiero es relativamente bajo, por los costes de las opciones obtenidos. Si se analizaran otros tipos de valores como los mercados de divisas extranjeras o criptomonedas, estos tendrían variables sumamente distintas y, por consiguiente, diferentes opciones de compra y venta.
